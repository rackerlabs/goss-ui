...
export const GCPSettingsSection = ({ project, jobs, setJobs, vms }) => {
  ...
  const ConfigureBackup = ({}) => {
    return <Page.MainBodySection title="Backup"></Page.MainBodySection>;
  };
        ...
        <Table.Row data-testid="monitoring-table-row">
          ...
          <Table.Cell compact={true}>
            ...
            >
              ...
              <strong className={'monitoring-table-row-name'} data-testid="monitoring-table-row-name">
                {tagGroup.name}
              </strong>
            </Button>
          </Table.Cell>
          <Table.Cell compact={true} data-testid="monitoring-table-row-create-date">
            {tagGroup.createDate}
          </Table.Cell>
          <Table.Cell compact={true} data-testid="monitoring-table-row-last-modified-date">
            {tagGroup.lastModifiedDate}
          </Table.Cell>
          <Table.Cell compact={true} data-testid="monitoring-table-row-editor-email">
            {tagGroup.editorEmail}
          </Table.Cell>
        </Table.Row>
        <MonitoringConfigToggleRow
          tagGroup={tagGroup}
          isShowingMonitoringConfig={isShowingMonitoringConfig}
          setCanModifyMonitoringRow={setCanModifyMonitoringRow}
          canModifyMonitoringRow={canModifyMonitoringRow}
        />
        <MonitoringConfigSettingsRow
          tagGroup={tagGroup}
          isShowingMonitoringConfig={isShowingMonitoringConfig}
          canModifyMonitoringRow={canModifyMonitoringRow}
          configValuesPassThrough={configValuesPassThrough}
  ...
  const MonitoringConfigToggleRow = ({
    tagGroup,
    isShowingMonitoringConfig,
    setCanModifyMonitoringRow,
      ...
      <Table.Row style={{ display: isShowingMonitoringConfig ? 'table-row' : 'none' }}>
        <Table.Cell colSpan={5} className={'noBorder'}>
          <div>
            <Button
              onClick={() => {
                setCanModifyMonitoringRow(!canModifyMonitoringRow);
              }}
              data-testid="display-monitoring-config-modify"
            >
              Modify
            </Button>
          </div>
  ...
  const ConfigurePatching = ({}) => {
    return <Page.MainBodySection title="Patching"></Page.MainBodySection>;
  };
  ...
  if (vms.length == 0) {
    return (
      <Page.MainBody>
        ...
        <br />
        <div>
          To get started, see your <a href="#">unenrolled instances on the services tab.</a>
        </div>
      </Page.MainBody>
    );
  } else {
